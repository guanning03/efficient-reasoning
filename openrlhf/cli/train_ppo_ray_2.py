...
def train(args):
    ...
    actor_bundles = [{"GPU": 0.3} for _ in range(args.actor_num_nodes * args.actor_num_gpus_per_node)]
    bundles.extend(actor_bundles)
    
    # Reference model resources  
    ref_bundles = [{"GPU": 0.1} for _ in range(args.ref_num_nodes * args.ref_num_gpus_per_node)]
    bundles.extend(ref_bundles)
    
    # Critic model resources (if enabled)
    if args.critic_pretrain:
        critic_bundles = [{"GPU": 0.3} for _ in range(args.critic_num_nodes * args.critic_num_gpus_per_node)]
        bundles.extend(critic_bundles)
    
    pg = placement_group(bundles, strategy="SPREAD")
    ray.get(pg.ready())

    # 使用 placement group 初始化模型
